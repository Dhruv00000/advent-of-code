...
grid_BinaryState = numpy.zeros((1000, 1000), dtype = numpy.int8)
grid_LightLevel = numpy.zeros((1000, 1000), dtype = numpy.int8)

for instruction in instructions:

    splitList = instruction.split(" through ")
    secondConer: list = splitList[1].split(",")
    firstCorner = splitList[0].split(" ")[-1].split(",")
    ...
    grid_LightLevel[grid_LightLevel == -1] = 0
